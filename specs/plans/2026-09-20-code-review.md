...
*Status 2026-09-21 (#13): TcUnit is referenced and 138 cases pass on the user-mode runtime. Every block now has golden tests with hard tolerances against
the port and against erfa (new suites for `FB_PRECESS`, `FB_RADEC2HADEC`, `FB_HADEC2RADEC`, `FB_SUNPOS` and the pole / zenith / RA-wrap /
out-of-range cases of `FB_EQ2HOR` / `FB_HOR2EQ`, generated by `testing/gen_block_tests.py`). The 40" claim is marked as unreproduced in the README and
the notebook. The notebook was re-run on the user-mode runtime (500 cases: PLC vs astropy median 0.31", max 1.09" as great-circle separation; no 40" outliers,
largest raw azimuth difference 3.8") and then replaced by `testing/check_plc_vs_astropy.py` (thresholds, exit code, `--net-id`, offline). New finding: `FB_HADEC2RADEC` at exactly dec = ±90 is 4.7" / 39" off (see AstroBROTTests/README.md).*

...
.DESCRIPTION
  AstroBROTTests references TcUnit as a placeholder ("TcUnit, * (www.tcunit.org)"), which only resolves if
  ...
  next TcBuild run fail with exit code 1 (see specs/plans/2026-09-15-twincat-ci-investigation.md in the BROTLib repository).
#>
[CmdletBinding()]
param(
    [string]$Solution = (Join-Path $PSScriptRoot '..\AstroBROTTests.sln'),
    [string]$Library  = (Join-Path $PSScriptRoot '..\vendor\tcunit.library'),
    [string]$PlcTreePath = 'TIPC^AstroBROTTests^AstroBROTTests Project^References'
)

<#
.SYNOPSIS
  Deploys AstroBROTTests to a TwinCAT runtime, runs the TcUnit suites and reports the result.
...
.DESCRIPTION
  ...
  reports the counters. Build first (TcBuild build AstroBROTTests.sln) so the compiled project is current.
...
[CmdletBinding()]
param(
    [string]$Solution    = (Join-Path $PSScriptRoot '..\AstroBROTTests.sln'),
    [string]$TargetNetId = '192.168.4.1.1.1',
    [string]$PlcTreePath = 'TIPC^AstroBROTTests',
...
try {
    ...
    for ($i = 0; $null -eq $sysMan; $i++) {
        ...
        # Projects.Count can already be 1 while Item(1) is still $null, and calling a method on $null is not a COM
        # error, so guard inside the retried block instead of letting it throw
        $sysMan = Invoke-Retry { $projects = $dte.Solution.Projects; if ($projects -and $projects.Count -ge 1) { $project = $projects.Item(1); if ($project) { ,$project.Object } } }
...
if ($cases -eq 0) {
    Write-Host 'No test case ran: the suites are registered but no TEST() call was reached.'
    exit 2
}
if ($failed -gt 0) {
